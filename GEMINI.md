...
4. **All backend calls go through the BFF (server-side), never browser → backend
   directly.** Every endpoint is reached at same-origin `/api/<path>` (e.g.
   `/api/auth/login`, `/api/my/profile`). The catch-all proxy
   `app/api/[...path]/route.ts` forwards to the backend server-side, injecting
...
8. **Single root tree — never create a `src/` directory.** All code lives directly
   under the repo root. Path alias is `@/* → ./*`. See `docs/FRONTEND_STRUCTURE.md`
   for the canonical directory map and "where does X go?" rules.
...
The browser **never** talks to the backend directly and **never** holds a token.
All traffic flows: **browser → same-origin `/api/<path>` → BFF proxy → backend**.

**The solid function is already built — use it, don't reinvent it:**

1. **`app/api/[...path]/route.ts`** — the catch-all proxy. It forwards *every*
   endpoint to the backend, injecting `Authorization: Bearer` from the httpOnly
   `access_token` cookie, auto-refreshing once on 401, and harvesting any tokens
   out of auth responses into httpOnly cookies (stripping them from the body).
   **You do NOT add a route handler per endpoint — this one file covers them all.**
2. **`server/backend.ts`** — server-only core (cookies, refresh, URL builder).
   Never import it from a Client Component.
3. **`@/lib/http`** — the browser client. Call it with a path **relative to `/api`**:
   ```ts
   import { http } from "@/lib/http";
   const profile = await http.get<MyProfile>("my/profile");      // → GET  /api/my/profile
   await http.post("juristic-requests", { registrationId });       // → POST /api/juristic-requests
   await http.post("auth/context", { juristicId });                // → context switch
   ```
   Wrap these in React Query hooks (`hooks/*`); never scatter raw `fetch()`.

**How to add a new endpoint:** you don't add anything to the proxy. Just call
`http.<verb>("<path-without-/api>")` from a hook. The path maps 1:1 to the backend
route (the backend mounts everything under `/api`, the proxy prepends it).

**Config:** `BACKEND_INTERNAL_URL` (server-only env, no `NEXT_PUBLIC_`) is the backend
origin. The old `NEXT_PUBLIC_API_URL` + `lib/api.ts` (direct axios) are **deleted** —
use `@/lib/http` only.

**Why:** tokens in httpOnly cookies are invisible to JS (XSS-safe); the backend
origin and refresh logic stay server-side; same-origin `/api/*` means no CORS.
...
- **`ChartContainer` must always receive an explicit `id` prop.** The `BackOfficeNavbar`
  ...
  See `docs/FRONTEND_IMPLEMENTATION_STATUS.md` for the full incident record.
