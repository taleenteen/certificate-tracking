...
const STATUS_BADGE: Record<AdminTaskStatus, { bg: string; text: string; border: string }> = {
  WAITING_ASSIGNMENT: { bg: 'bg-yellow-50', text: 'text-yellow-700', border: 'border-yellow-200' },
  ...
  PENDING_REVIEW:     { bg: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-200' },
...
function StatusBadge({ status }: { status: AdminTaskStatus }) {
  ...
  return (
    <span className={`inline-block px-2.5 py-1 rounded-[6px] text-[12px] font-semibold border ${c.bg} ${c.text} ${c.border}`}>
...
export function InspectionsPage() {
  ...
  const { data: tasks = [], isLoading, isError, refetch } = useInspectionTasks(
    statusFilter !== 'ALL' ? statusFilter : undefined,
  );
  const { data: inspectors = [] } = useUsers({ role: 'inspector' });

  const [isCreateOpen, setIsCreateOpen] = React.useState(false);
  const [assigningTask, setAssigningTask] = React.useState<InspectionTaskSummary | null>(null);

  const waitingCount = tasks.filter((t) => t.status === 'WAITING_ASSIGNMENT').length;
  ...
  return (
    <div className="space-y-[16px] text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        <Card className="rounded-[12px] border border-gray-200 shadow-smooth-low bg-white p-4 flex gap-4 items-start relative overflow-hidden">
          ...
          <div className="flex-1 space-y-1">
            ...
            <p className="text-3xl font-bold text-main pt-1">
              {isLoading ? <RefreshCw className="animate-spin size-5 inline-block" /> : waitingCount}
            </p>
          </div>
          {waitingCount > 0 && (
        ...
        <Card className="rounded-[12px] border border-gray-200 shadow-smooth-low bg-white p-4 flex gap-4 items-start relative overflow-hidden">
          ...
          <div className="flex-1 space-y-1">
            ...
            <p className="text-3xl font-bold text-main pt-1">
              {isLoading ? <RefreshCw className="animate-spin size-5 inline-block" /> : inProgressCount}
            </p>
          </div>
        </Card>
        ...
        <Card className="rounded-[12px] border border-gray-200 shadow-smooth-low bg-white p-4 flex gap-4 items-start relative overflow-hidden">
          ...
          <div className="flex-1 space-y-1">
            <h3 className="text-[13px] font-semibold text-placeholder">งานเสร็จสิ้น</h3>
            <p className="text-[11px] text-emerald-600 font-medium leading-none">สถานะ: อนุมัติแล้ว</p>
            <p className="text-3xl font-bold text-main pt-1">
              {isLoading ? <RefreshCw className="animate-spin size-5 inline-block" /> : completedCount}
            </p>
          </div>
        </Card>
        ...
        <Card className="rounded-[12px] border border-gray-200 shadow-smooth-low bg-white p-4 flex gap-4 items-start relative overflow-hidden">
          ...
          <div className="flex-1 space-y-1">
            <h3 className="text-[13px] font-semibold text-placeholder">เจ้าหน้าที่ในสังกัด</h3>
            <p className="text-[11px] text-purple-600 font-medium leading-none">ผู้ตรวจสอบทั้งหมด</p>
            <p className="text-3xl font-bold text-main pt-1">{inspectors.length}</p>
          </div>
        </Card>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[950px]">
            <thead>
              <tr className="bg-fuji-light/30 border-b border-gray-200">
                ...
                <th className="px-[16px] py-[12px] text-[12px] font-bold text-placeholder w-[130px]">ประเภท</th>
                <th className="px-[16px] py-[12px] text-[12px] font-bold text-placeholder">ชื่อกิจการ</th>
                <th className="px-[16px] py-[12px] text-[12px] font-bold text-placeholder w-[180px]">เจ้าหน้าที่รับผิดชอบ</th>
                <th className="px-[16px] py-[12px] text-[12px] font-bold text-placeholder w-[110px]">วันนัดตรวจ</th>
                <th className="px-[16px] py-[12px] text-[12px] font-bold text-placeholder w-[130px]">สถานะ</th>
                <th className="px-[16px] py-[12px] text-[12px] font-bold text-placeholder w-[120px]">จัดการ</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                filteredTasks.map((task) => (
                  <tr key={task.id} className="border-b border-fuji-light/50 hover:bg-fuji-light/5 transition-colors">
                    <td className="px-[16px] py-[12px] text-[13px] text-brand-primary font-bold font-mono">{task.taskNo}</td>
                    ...
                    <td className="px-[16px] py-[12px] text-[13px] text-main font-medium">{task.business.nameTh}</td>
                    <td className="px-[16px] py-[12px] text-[13px]">
                      {task.assignee ? (
                        <span className="text-main font-medium">{task.assignee.fullName}</span>
                      ) : (
      ...
      {assigningTask && (
        <InspectionFormModal
          open={!!assigningTask}
          onOpenChange={(v) => { if (!v) setAssigningTask(null); }}
          mode="assign"
          task={assigningTask}
        />
      )}
