...
export function SuperAdminHeader() {
  ...
  return (
    <header className="h-[80px] bg-background border-b border-border-default flex items-center justify-between px-8 shadow-sm sticky top-0 z-10">
      <h1 className="text-[22px] font-bold text-text-primary">{title}</h1>

      
      <div className="flex items-center gap-5">
         <button className="size-11 rounded-full bg-fuji-light flex items-center justify-center text-text-secondary hover:bg-fuji-soft transition-colors border border-border-default relative">
            <Bell className="size-5" />
            <span className="absolute top-2.5 right-2.5 size-2 bg-semantic-critical rounded-full border border-background"></span>
         </button>
         
         <div className="h-12 px-4 py-2 border border-border-default rounded-square flex items-center gap-3 hover:bg-fuji-light cursor-pointer transition-all">
            ...
            <div className="flex flex-col text-left">
              ...
              <span className="text-[11px] text-text-placeholder font-medium mt-1">
                {user?.roles?.includes('superadmin') ? 'Super Admin' : 'ผู้ดูแลระบบ'}
              </span>
            </div>
            <ChevronDown className="size-4 text-text-placeholder ml-1" />
         </div>
      </div>
    </header>
  );
